//https://discussions.unity.com/t/make-an-object-face-current-cameras-worldpoint-target/228476 
// collision detection: https://docs.unity3d.com/ScriptReference/Collision-gameObject.html 
// audio reference: https://youtu.be/lBTtzqfaNdM?si=Eb341b51sBJjU1L6
...
// make object face camera using LookAt: https://discussions.unity.com/t/make-an-object-face-current-cameras-worldpoint-target/228476
...
public class RockController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        secondPosition = new Vector3(this.transform.position.x - 10, this.transform.position.y, this.transform.position.z + 5); //might have to play with these values
    ...
    void OnCollisionEnter(Collision collision)
    {
        ...
        //put the name of the window object in the " " !
        if (collision.gameObject.name == "FauxWindow")  //if the rock collides with the window, do:
        {
            ...
            audioSource.PlayOneShot(audioClip); //play sound effect
        }

    }

    /* keyboard input controls used for debug -- ignore these, the code is messy and not directly used in the system, 
     * but I'm saving the code for posterity */
    
    /*
    void checkKeyDebugControls() 
    {
        //add an impulse force to the rock so it flies at the camera
        if (Input.GetKey(KeyCode.Space)) //up arrow key pressed
        {
            body.angularVelocity = Vector3.zero;
            //body.velocity = Vector3.zero;
        ...
        if (Input.GetKeyDown(KeyCode.O)) //up arrow key pressed
        {
            //set rock's linear and angular velocity to 0, aka stop rock's motion
            body.angularVelocity = Vector3.zero;
            body.velocity = Vector3.zero;
        ...
        if (Input.GetKey(KeyCode.P)) //up arrow key pressed
        {
